...
with col2:
    if uploaded_file and identify_btn:
        ...
        # 显示初步结果
        # 1. 调整列宽比例：给学名分配更多空间 (例如 3:1 或 2:1)
        c1, c2 = st.columns([3, 1]) 
        
        with c1:
            # 2. 手动模拟 Metric 样式，但使用 Markdown 实现自动换行
            st.caption("识别学名")  # 显示灰色小标题
            st.markdown(f"#### {latin_name}")  # 使用三级或四级标题显示学名，会自动换行
            
        with c2:
            # 置信度比较短，保持用 metric 即可
            st.metric("视觉置信度", f"{confidence:.1f}%")
